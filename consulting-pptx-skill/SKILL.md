...
7. **上流ロジックの変更は必要最小限にする。** このコピーではWindows互換のパス・依存解決と、自己検査に落ちていた同梱サンプルだけを修正している。プロジェクト固有の学びは、まず作業コピー側のルール追補として提案し、再利用価値と副作用を確認してからSkill本体へ反映する。

...
Deliver the variants and findings following the [SKILL](SKILL.md) completion boundary. If a choice is still needed, present the usable prototype for that decision. Only when production implementation is also requested, adapt the selected design with production tests and error handling:

- **Sub-shape A**: fold the winner into the existing page; drop the losing variants and the switcher from main.
- **Sub-shape B**: promote the winning variant to a real route; drop the throwaway route and the switcher from main.

Preserve useful variants as a clearly marked reference. Creating a branch, committing or updating an issue is conditional on the authorized workflow, not a requirement for completing the prototype.

...
On Windows, create the collision-free directory with PowerShell (`New-Item` under `[System.IO.Path]::GetTempPath()`) and invoke the bundled Codex Python, `py -3`, or another available Python 3 executable instead of assuming `python3` is on `PATH`.
